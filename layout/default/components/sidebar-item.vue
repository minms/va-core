  ...
  <div class="sidebar-item" :class="{'root': root}">
    <template v-if="!menu.children || menu.children.length === 0">
      <sidebar-item-link :to="realPath(menu.path)">
        <el-tooltip effect="dark" :disabled="root && !collapse" :content="menu.label" placement="right">
    ...
    <template v-else>
      ...
      <el-submenu v-else ref="subMenu" :index="realPath(menu.path)" popper-append-to-body>
        <template slot="title">
          <span class="icon" v-if="menu.icon" :class="menu.icon"></span>
          <span class="label">{{ menu.label }}</span>
        </template>
